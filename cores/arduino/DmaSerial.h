 ...
 */
uint8_t get(uint8_t delimiter, uint8_t* bytes, uint8_t length);
...
int print(char* buff, uint8_t length);

int readBytesUntil(char delimiter, char* buffer, uint8_t length);
 ...
 */
void disable() {
								uart->UART_PTCR = UART_PTCR_TXTDIS; uart->UART_PTCR = UART_PTCR_RXTDIS;
};
 ...
 */
void enable() {
								uart->UART_PTCR = UART_PTCR_TXTEN; uart->UART_PTCR = UART_PTCR_RXTEN;
};
